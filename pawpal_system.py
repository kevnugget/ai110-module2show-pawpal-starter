...
class Scheduler:
    ...
    def prioritize_tasks(self, tasks: List[Task], now: Optional[datetime] = None) -> List[Task]:
        now = now or datetime.now()
        return sorted(tasks, key=lambda t: t.estimate_urgency(now), reverse=True)

    def schedule_day(self, date: datetime) -> List[Task]:
        """Return a list of tasks ordered by urgency for the given day."""
        day_start = datetime(year=date.year, month=date.month, day=date.day)
        day_end = day_start + timedelta(days=1)
        candidates = [t for t in self.tasks if not t.completed and day_start <= t.due_at < day_end]
        return self.prioritize_tasks(candidates, now=day_start)
